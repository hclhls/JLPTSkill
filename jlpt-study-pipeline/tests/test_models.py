...
def test_video_field_config_defaults():
...
def test_video_field_config_custom_values():
    config = VideoFieldConfig(
        term_count=3,
        meaning_count=2,
        example_count=2,
...
def test_video_field_config_ordered_fields():
    config = VideoFieldConfig(
        term_order=3,
        meaning_order=1,
        example_order=2,
    ...
    # Should be sorted by order value: meaning (1), example (2), term (3)
    assert ordered == [
        ("meaning", 1),
        ("example", 2),
        ("term", 3),
