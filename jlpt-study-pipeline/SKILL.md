...
Use `--example-style phrase` to use the short `example_ja_phrase` field instead of the full sentence for TTS audio, video subtitles, and Anki card backs. If an entry has no `example_ja_phrase`, it falls back to `example_ja` automatically. **Always ask the user which style they prefer** before running the pipeline.
